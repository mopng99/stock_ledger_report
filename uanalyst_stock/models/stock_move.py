...
class StockMove(models.Model):
    ...
    total_line_cost = fields.Monetary(string="Total Line Cost", compute='_compute_product_line_cost',
                                   store=True)
    ...
    def _compute_product_line_cost(self):
        ...
        """
        for record in self.filtered(lambda r: r.product_id and r.product_uom):
            per_uom_cost = record.product_id.uom_id._compute_price(record.product_id.standard_price,
                                                                   record.product_uom)
            record.total_line_cost = per_uom_cost * record.quantity_done
